...
int main(int argc, char** argv) {
    ...
    double X_THRESHOLD_CONTACT = 1, X_THRESHOLD_SLIDE = -10, WIGGLE_OFFSET = 0.01, X_THRESHOLD_PUSH = 50, X_THRESHOLD_TORQUE = 0.3, VIRT_ATTR_ROT_MOVE = 0.8;
    double has_hit = 0;
    ...
    while(ros::ok()) {
        ...
        switch (state) {
            ...
            case 1:
                ROS_INFO("State 2");
				//TODO fix torque
				if(abs(ft_robot_frame.torque.x) < X_THRESHOLD_TORQUE) { //  Check if the torque would be positive
                    ...
                    Eigen::Affine3d ROT_MAT; // about 45 degrees
                    ROT_MAT(0,0) = 1;
                    ROT_MAT(0,1) = 0;
                    ROT_MAT(0,2) = 0;
                    ROT_MAT(1,0) = 0;
                    ...
                    // Do rotation
                    Eigen::Affine3d new_virt_attr_rot = curr_pose_rot * ROT_MAT;
                    
                    // Convert new orientation to quaternion
                    Eigen::Quaterniond new_virt_attr_quat(new_virt_attr_rot.linear());
                    ...
                    acc_gain_vec.data[3] = 1; // DO WE WANT IT TO ACCOMODATE IN THE WAY WE ROTATE?
